...
public partial class DataStoreTests
{
    ...
    public void DeleteUnreferencedIssues()
    {
        ...
        dataStore.Connection.Insert(new Search { Name = "Hello", SearchString = "Shooting star" });
    ...
    [TestMethod]
    [TestCategory("Unit")]
    public void DeleteBeforeSearchIssue()
    {
        using var dataStore = new DataStore("TestStore", TestHelpers.GetDataStoreFilePath(TestOptions), TestOptions.DataStoreOptions.DataStoreSchema!);
        Assert.IsNotNull(dataStore);
        dataStore.Create();
        ...
        dataStore.Connection.Insert(issues[1]);
        dataStore.Connection.Insert(new Search { Name = "Hello", SearchString = "Shooting star" });
        dataStore.Connection.Insert(new SearchIssue { Issue = 1, Search = 1, TimeUpdated = now.ToDataStoreInteger() - 200 });
        dataStore.Connection.Insert(new SearchIssue { Issue = 2, Search = 1, TimeUpdated = now.ToDataStoreInteger() - 100 });

        var dsSearch = dataStore.Connection.Get<Search>(1);

        SearchIssue.DeleteBefore(dataStore, dsSearch, now - TimeSpan.FromTicks(150));
    ...
    [TestMethod]
    [TestCategory("Unit")]
    public void DeleteBeforeSearchPullRequests()
    {
        using var dataStore = new DataStore("TestStore", TestHelpers.GetDataStoreFilePath(TestOptions), TestOptions.DataStoreOptions.DataStoreSchema!);
        Assert.IsNotNull(dataStore);
        dataStore.Create();
        ...
        dataStore.Connection.Insert(new SearchPullRequest { PullRequest = 1, Search = 1, TimeUpdated = now.ToDataStoreInteger() - 200 });
        dataStore.Connection.Insert(new SearchPullRequest { PullRequest = 2, Search = 1, TimeUpdated = now.ToDataStoreInteger() - 100 });

        var dsSearch = dataStore.Connection.Get<Search>(1);
        SearchPullRequest.DeleteBefore(dataStore, dsSearch, now - TimeSpan.FromTicks(150));
